...
interface IDebtTakerFactory {
    function newDebtTaker(address vault, address recipient, address manager, uint256 threshold, address ajnaPool, uint256 maxBorrowingRate) external returns (DebtTaker);
}
...
contract DebtTakerTest is Test {
    ...
    IERC20Pool public ajnaPool = IERC20Pool(0x7491D5e4CfF624eaA047eaB115ad2Ca7541D3ed5); // sDAI/DAI pool in ajna
    address public recipient = vm.addr(2);
    address public manager = vm.addr(3);
    address public lender = vm.addr(4);
    address public angel = vm.addr(5);
    uint256 public threshold = 100 ether;
    uint256 public maxBorrowingRate = 0.1 ether; // 10%

    function setUp() public {
        originalDebtTaker = new DebtTaker();
        debtTakerFactory = IDebtTakerFactory(deployer.deployContract('DebtTakerFactory', abi.encode(originalDebtTaker)));
        debtTaker = debtTakerFactory.newDebtTaker(address(vault), recipient, manager, threshold, address(ajnaPool), maxBorrowingRate);
    ...
    function testDepositAsset() public {
        ...
        assertEq(IERC20(debtTaker.token()).balanceOf(address(recipient)), 100 ether);
